...
export async function bookingPrefill(authData) {
  const storedAuth = authData || readJsonStorage("maya_auth") || readJsonStorage("tg_auth_data");
  const sessionToken = readStorage("web_session_token");
  if (!storedAuth && !sessionToken) return { success: false, error: "no_auth" };

  const headers = { "Content-Type": "application/json", Accept: "application/json" };
  if (sessionToken) headers["X-Session-Token"] = sessionToken;
  const res = await fetch(`${PROXY}?action=booking_prefill`, {
    method: "POST",
    headers,
    body: JSON.stringify({ auth_data: storedAuth, session_token: sessionToken }),
  });
  return res.json().catch(() => ({ success: false }));
}
